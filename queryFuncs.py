...
def _random_n_rows(table_name, n_row):
    return f'SELECT * FROM "{table_name}" ORDER BY RANDOM() LIMIT {n_row};'


def random_n_rows(time, n_row, db_file_path, num_data_operand=None):
    connection = Connection(db_file_path)
    cursor = connection.cursor()
    cursor.execute(get_list_table())
    list_table = [t_[0] for t_ in cursor.fetchall() if t_[0].startswith(str(time))]
    ...
    for table in list_table:
        query = _random_n_rows(table, n_row)
        print(query)
        cursor.execute(query)
        list_value = cursor.fetchall()
        n_op = int(table.split("_")[1])
    ...
    data = DataFrame(list_of_list_value, columns=list_col)
    connection.close()
    return data.sample(frac=1).iloc[:n_row]
